...
jenkins components footnote:[We noticed many ++Descriptor++s rely on manual parsing of +JSONObject+. We will need to fix them.] do rely on them and as such offer a 1:1 match between internal data structure and web UI configuration forms.
