...
4. Permit the entire log to be sent to a pluggable external service:
   in the reference implementation, Amazon CloudWatch Logs.
...
and could be integrated with these changes via
...
Log lines produced on the agent side are temporarily buffered
before being streamed to the master in relatively large chunks,
to avoid putting too much load on the Remoting channel,
so remote tasks need to `flush` before exiting.
...
The reference implementation creates JSON-format records to be sent to CloudWatch Logs.

`ConsoleNote` instances need to be created on the master side so that they may be signed.
Signed and serialized note constants may be transmitted to agents to handle this in simple cases.
Many traditional uses of this API, however, can be replaced by a new `TaskListenerDecorator` interface
that improves on the old `ConsoleLogFilter` extension point.
`ConsoleAnnotatorFactory` has also been improved for Pipeline use and can replace some uses of `ConsoleNote`.
The `timestamper` and `ansicolor` plugins have been reworked accordingly.
...
Yet CloudWatch Logs does not guarantee that a given record
has been committed to permanent storage when the log event is sent,
and outbound records and queued and coalesced in a buffer,
...
A special marker was inserted into the text to indicate the `FlowNode.id` associated with each line.

This however had performance issues, and was ill suited to the CloudWatch Logs implementation.
...
Therefore a text marker was introduced,
as it adds minimal space overhead and does not interfere with legibility.
This in turn was later replaced even in the default file-based implementation (see above).
...
JEP-207 tracks some, though not all, of these proposals.
...
Plugins remoting a `TaskListener` need to take care to `flush` the stream after completing a task,
as output may have been buffered.

Plugins calling `Run.getLogFile` will produce a warning.
If the actual log is remote, a temporary file will be produced with a copy of the log text.
...
The reference implementation uses IAM to create temporary access tokens for use by agents.
See [this detailed explanation](https://github.com/jenkinsci/pipeline-cloudwatch-logs-plugin/blob/8e5024f02698db2aeed90374a92b90960fa32f9b/README.md#security).

== Infrastructure Requirements

Properly testing the reference implementation would require a CI server to have an AWS account as discussed below.
...
Tests for SaaS-based implementations such as CloudWatch require either mocks,
...
A disabled test in `workflow-job` demonstrates a typical heavy log load scenario for the file-based implementation.
