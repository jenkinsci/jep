...
For similar reasons to Spring https://spring.io/blog/2019/01/15/spring-framework-s-migration-from-jira-to-github-issues/#background:
...
- link:https://github.com/timja/jenkins-gh-issues-poc-06-18/issues[Import #1 2022/06/18]
- link:https://github.com/timja-org/jenkins-gh-issues-poc-11-04/issues[Import #2 2025/11/04]
- link:https://github.com/timja-org/jenkins-gh-issues-poc-11-23/issues[Import #3 2025/11/23]
- link:https://github.com/lemeurherve/jira-issues-importer[Migration tool]

== FAQ

=== How do I find issues I've reported or commented in on Jira?

In the GitHub issues search bar add `reporter=danielbeck`.

E.g. https://github.com/timja-org/jenkins-gh-issues-poc-11-13/issues?q=is%3Aissue%20reporter%3Ddanielbeck

We've added metadata hidden in the issue body and comments to aid searching:
...
To find issues you've commented on: `comment_author=danielbeck`

e.g. https://github.com/timja-org/jenkins-gh-issues-poc-11-23/issues?q=is%3Aissue%20comment_author%3Ddanielbeck

=== I watch quite a few issues in Jira, how can I quickly map Jira issues to GitHub issues?

There's a report that will be produced as part of the migration that maps Jira issue keys to GitHub issue numbers.
...
=== What is happening with issue attachments on Jira?

GitHub doesn't support automatically uploading files to issues for security reasons.

As part of the migration Jira attachments have been uploaded to https://github.com/jenkinsci/attachments-from-jira-issues-core-cli for `core` and `cli` component issues (1.39Go), and their original link replaced by those mappings in imported issues and comments.

A mapping to these attachments file can be found at https://github.com/jenkinsci/artifacts-from-jira-issues/blob/main/mappings/jira_attachments_repo_id_filename.txt.

Attachments from other components won't be stored in the same repository as GitHub has a maximum size of 2Go per repository.

=== The description doesn't render well in some cases, what gives?

Jira uses a different markup language to GitHub.
We're using an HTML export from Jira which does a pretty good job.
We've also tried Jira markup to Markdown conversion but that had issues as well.

The majority of issues render fine, the ones that don't look good are generally when there's HTML in the Jira issue description, e.g. XML content copied directly in the issue.

To compensate for these issues, the raw content is stored in the body of the imported issues and comments themselves, behind a details block.

We've accepted that some issues will need manually fixing up as they are reviewed.

=== Will a monorepo for all issues be used, e.g. jenkinsci/issues?

No, each repository will have its own issue tracker. Searches and projects can be used to group issues across repositories.

e.g:
* `core` component to jenkinsci/jenkins
* `cli` component to jenkinsci/jenkins - label to associate with CLI
* `packaging` component to jenkinsci/packaging

=== How will issues with multiple components be handled?

Any issues with the `core` or `cli` component will go to jenkinsci/jenkins and they will be labelled with the other components.

Going forward issues should be filed in the most appropriate repository.

If it's an issue in a plugin then it should be filed there first.

If an issue is created but needs fixes in another repository then:
...
* The issue can be moved
  * This should only be done if it was misfiled, e.g. if the issue keeps being hit by people using a plugin but its caused by another plugin then it may be worth keeping it in the plugin that people hit it with so the issue is the most discoverable.

=== How can I get to a GitHub issue from a Jira issue key?

There is a service available that redirects from a Jira issue key to the corresponding GitHub issue.

https://issue-redirect.jenkins.io/issue/1

Will redirect to the GitHub issue corresponding to JENKINS-1.

Alternatively a comment is added to each comment on Jira with a link.
The issue-redirect service is used throughout jenkins.io documentation.
