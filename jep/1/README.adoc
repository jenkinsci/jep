...
feature proposal footnoteref:[repo, https://github.com/jenkinsci/jep].o
...
A JEP may have more than one Author, especially once it is finalized and.
...
. **<<start, Inital Discussion>>** - Andrea has an idea for new feature emails jenkinsci-dev@googlegroups.com.
  ...
  She chooses to be the "<<Author>>" for this potential JEP.
...
To submit a JEP for approval as Draft, the JEP author should:
...
and send the submission back to the author for revision.
...
appropriate "main" code repository, the JEP author will change
...
. Re-target the original author's pull request to the feature branch
