...
when refering the one or more people in the role of "Author" of a JEP.
...
. **Submission** - Andrea writes up the proposal using the JEP document template as a guide, 
  including a supporting documentation and a reference implmentation if needed.
...
via Pull Ruquest.  It may update the
