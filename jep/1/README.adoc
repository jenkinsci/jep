...
. **Security** - All JEPs must include a section ddescribing their impact
  on the security of affected components, and, if any, how these are dealt with.
  This includes discussion on potential security issues and how they are prevented,
  as well as how the proposed changes interact with existing permissions, authentication, authorization, etc.
...
. **Infrastructure Requirements** -
  ...
  and assigning responsibilty for continuing maintenance.
  Each JEP must explain the scope of infrastructure change with sufficient detail
  to allow their initial and on-going cost (in both time and money) can be estimated.
