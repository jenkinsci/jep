...
help write, implement, discuss, review, or offer feedback about that JEP.
One contributor might do only one or any combination of these
of these things during any part of the life of a JEP.
...
Each JEP has one "Reviewer".

The JEP Reviewer is a contributor who will make the final decision whether to accept a JEP.
Upon request of the sponsor, the reviewer will:

* Verify the JEP has achieved consensus within the community,
  including documentation of dissenting opinions
* Consult with other area experts
* Ensure the JEP is sufficiently complete
  to be implemented without further major additions or changes
...
. **<<start, Inital Discussion>>** - Andrea has an idea for new feature and emails it jenkinsci-dev@googlegroups.com.
  ...
  A well-known contributor voluteers to be the the "<<Reviewer>>"
  and there is general agreement they are they a good choice.
...
. **<<review, Review>>** - The <<Reviewer>> reviews the JEP and decides to accept it,
...
on the jenkinsci-dev@ mailing list explaining their decision.
...
Rejected JEP to Draft status, but this is at the discrection of the reviewer.
...
Except for the decision by a reviewer to accept or reject a JEP,
...
. **Sponsor** -- <<Reviewer>> of the JEP, the individual will make the final decision to approve or reject a JEP.
...
The Python process has a role called the BDFL,
the "link:https://en.wikipedia.org/wiki/Benevolent_dictator_for_life[Benevolent Dictator for Life]".
The BDFL acts as reviewer and resolver of disuputes.
...
contributors, editors, and reviewers.
...
we decided to condsider asciidoc style and formatting guidelines in a later JEP.
