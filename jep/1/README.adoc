...
The <<BDFL>> is the final authority for reviewing and resolving JEPs,
but may delegate that responsibility to another contributor,
...
A JEP's <<Sponsor>> cannot also be the BDFL Delegate for that JEP.

In all cases where this document refers to the "BDLF" making a decision regarding a JEP,
it means "the BDFL or BDFL Delegate for this JEP."
...
. **<<review, Review>>** - The <<BDFL>> reviews the JEP and decides to accept it,
...
Rejected::  [[rejected]]
A JEP can also be "Rejected" by the BDFL. Perhaps after all is said and done it
...
If the BDFL believes the JEP might be Accepted after some modification,
the BDFL will not reject the JEP, instead returning it Draft status
with feedback for the sponsor.
Upon the request of the sponsor, the BDFL may choose to return a
Rejected JEP to Draft status, but this is at the discretion of the BDFL.
...
Except for decisions by the <<BDFL>> to accept or reject a JEP,
...
If consensus cannot be achieved, contributors may request that the <<BDFL>> intervene.
...
If contributors believe a decision made by the BDFL in this manner is not in the best interests to Jenkins project,
