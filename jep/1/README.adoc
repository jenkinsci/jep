...
Sponsors have commmitter/write access on the JEP repository, but should
...
BDFL Delegates have commmitter/write access on the JEP repository, but should
only approve and merge pull requests for JEPs to which they are assigned.
...
When somone is no longer an active Sponsor or BDFL Delegate,
a JEP editor will remove them from the permission group.  
Editors will cleanup the the permission groups from time to time 
