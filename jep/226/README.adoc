...
This is followed by clearing the local database of fingerprints from Jenkins HOME ONLY if the writes are successful.

Following new methods are introduced in the `FingerprintStorage` class:

* `void localToExternalMigration(boolean cleanOnSuccess)`

** This method transfers the fingerprints from the local file system storage to the configured external fingerprint storage. `cleanOnSuccess` cleans the transferred fingerprints from local storage on success. The method has package level scope, not publicly available.

The above method can be called via the configuration page, and is available for use only when an external fingerprint storage plugin has been configured.
...
The following new method is introduced in `FingerprintStorage` class:

* `void externalToLocalMigration(boolean cleanOnSuccess)`

** This method transfers the fingerprints from the external storage to the local file system based fingerprint storage. `cleanOnSuccess` cleans the transferred fingerprints from local storage on success. The method has package level scope, not publicly available.
...
This can be done by an external to local migration from the old plugin, followed by a local to external migration with the new plugin.
