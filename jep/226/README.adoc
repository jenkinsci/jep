...
** Returns true if there's some data in the fingerprint storage.
...
Refactoring and improving `SaveableListener` is out of scope for this JEP.
...
* link:https://github.com/jenkinsci/redis-fingerprint-storage-plugin[Reference Implementation for Redis]
* link:https://github.com/jenkinsci/postgresql-fingerprint-storage-plugin[Reference Implementation for PostgreSQL]
