...
*  New APIs for `ClassFilter` management are blacklisted in the Script Security plugin
...
0. `Jenkins-ClassFilter-Whitelisted` manifest entry (link:https://github.com/jenkinsci/lib-jenkins-maven-embedder/pull/15[example for Maven])
0. By providing a custom `META-INF/hudson.remoting.ClassFilter` resource file in the plugin
0. By implementing the `CustomClassFilter` extension point and implementing `permits()` methods

The first two approaches do not require Jenkins core requirement update in plugins.
Old Hudson plugins may also need additional manifest properties to be processed as Jenkins plugins by the engine.
...
// oleg-nenashev: TODO: Clarify, which plugins have been tested and which plugins in the setup wizard were not covered by tests
...
// oleg-nenashev: This process is a bit harsh for a single JEP sponsor, but in this case the sponsor represents a company AFAIK.
// So I feel reasonable to set a maintenance quality bar taking a serious risk of regressions into account
...
* Issues with the `jep-200` label in Jenkins JIRA get first response within 2 business days (Monday-Friday)
* Eligible components: Any non-blacklisted plugin hosted within the `jenkinsci` organization
with more than `100` installations.
* _JEP-200 maintainer(s)_ may request additional information from the reporter,
but they are expected to provide a response within 2 business days after each iteration.
Finally, they are expected to communicate the triage outcome.
* Possible triage outcomes:
** Accepted - patch in the plugin
...
* For accepted issues _JEP-200 maintainer(s)_ schedule the fix and communicate the fix ETA to the reporter

=== Phase 3. LTS availability

Target Date: Mar 14, 2018
...
The weekly may be rejected if the release is not stable enough.
...
_JEP-200 maintainer(s)_ will be responsible to triage/fix issues till May 01, 2018.
