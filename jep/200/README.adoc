= JEP-200: Switch Remoting/XStream denylist to a permitlist
:toc: preamble
:toclevels: 3
ifdef::env-github[]
:tip-caption: :bulb:
...
| Title
| Switch Remoting/XStream denylist to a permitlist
...
This proposal switches the denylist to a permitlist of a few dozen entries:
...
* Any other class is accepted if it is explicitly mentioned in a list `permitlisted-classes.txt` bundled in Jenkins core. These consist of:
...
The permitlist has been built up by repeatedly running a large array of tests against a patched core and collecting relevant errors.
It is expected that the standard permitlist will be expanded somewhat as field reports arrive of less common code paths needing new entries.
...
Plugin developers will be able to permit the classes in their libraries/plugins using
...
0. By implementing the `CustomClassFilter` extension point, for dynamic permitlists—for example, all implementations of some interface.
...
** Upgrade guidelines will also provide permitlist management guidelines to plugin developers
...
** Accepted - update permitlist in the core (similar to link:https://github.com/jenkinsci/jenkins/pull/3120/files#diff-ff24cb984ddd641f49a22cc13a90cfd3[these cases]),
...
In practice it seems that very few plugins actually need to serialize any (third-party) types outside the permitlist.
Many such cases point to dubious design decisions, but to retain compatibility a few such entries are bundled in core.
Plugins or administrators can also expand the permitlist if regressions arise.
...
Messages sent from an agent to the Jenkins controller (unprompted, or responses to requests) are normally passed through a "callable permitlist" as of
link:https://jenkins.io/security/advisory/2014-10-30/[2014-10-30].
This permitlist is only applied _after_ deserializing the message, though, at which point it may be too late.
...
Outside security authorities have repeatedly urged the Jenkins team to switch to a permitlist.
...
In the other direction, it would be possible to reduce the size of the permitlist
...
but in fact the permitlist size increase needed to handle third-party classes with no deserialization methods is not dramatic,
...
which was already covered by a denylist entry (and now a lack of permitting as well).

=== Concern about the single permitlist approach

https://github.com/oleg-nenashev[@oleg-nenashev] raised a concern about using the same permitlist for Remoting and XStream:
...
There is an obvious risk that some plugins will have a legitimate need to serialize and deserialize third-party types not covered in the permitlist.
...
If new permitlist entries are needed after release, they can be added to core in weekly updates.
Plugins can also contribute their own permitlist (or even denylist) entries for third-party libraries they bundle,
as described in Extensibility above.

Finally, an individual administrator can define site-specific permitlist (or denylist) entries with a system property `hudson.remoting.ClassFilter`.
...
`jenkins.security.ClassFilterImpl.SUPPRESS_WHITELIST` disables the permitlist, logging violations, but keeps the denylist;
...
This permalink is printed to log messages appearing when a permitlist violation is encountered;
...
for example, that an example library class not currently included in the permitlist is rejected under the expected conditions.

A number of core tests had already been added during various advisories as mentioned in the motivation.
When the fallback to the original denylist is disabled, these continue to pass, indicating that the permitlist alone is a good defense.
...
though sufficient reasonable permitlist entries were added to not cause regressions for Blue Ocean even if that were not fixed.

In several cases, test failures and consequent permitlist additions highlighted poor design decisions in existing code.
For example, as of
...
Enforcing a permitlist in the baseline version of Jenkins might help guide them to the simpler solution.
