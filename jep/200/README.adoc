...
** After the discoveries in PCT (see below), the ATH results need to be reviewed carefully
** BDFL Delegate is waiting for info about what has been tested
(Jenkins WAR from link:https://github.com/jenkinsci/jenkins/pull/3120[Jenkins PR #3120] or a custom build with custom plugin versions)
* link:https://github.com/jenkinsci/plugin-compat-tester[Plugin Compatibility Tester (PCT)] has been executed for a limited plugin set for a sponsor's custom Jenkins WAR
** Not all plugins recommended in the Setup Wizard have been verified
** The verified plugins have been checked with the sponsor's special set of plugins, not with the latest releases of plugins in Jenkins update centers.
It cannot be considered as a reliable check since newer versions may contain other classes being serialized over Remoting/XStream.
...
** In the current state PCT does not run well against the Jenkins WAR packages with the reference implementation patch.
BDFL Delegate discovered several issues, which prevent him from running PCT.
These issues are aggregated in the link:https://issues.jenkins-ci.org/browse/JENKINS-48734[JENKINS-48734] EPIC.
