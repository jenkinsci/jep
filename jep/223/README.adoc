...
As we are opening the "/manage" page to one other permission than `Jenkins.ADMINISTER` we need to keep all class that extends ManagementLink secured. To do it we need to change the default value of getRequiredPermission() from:
...
This change is only removing the link that is displayed in the "/manage" page for users that don't have the related permission. Before this change the "/manage" page was only visible with a `Jenkins.ADMINISTER` permission, and after this change this permission still has the same content in the "/manage" page.

As it could break plugin compatibility we did an analysis on the plugin usage of `getRequiredPermission` or `requiredPermission` in jelly/groovy.


.Table Plugin usage analyses on ManagementLink.getRequiredPermission()
|===
...
|docker-plugin
|Change
|Does not override getRequiredPermission() in the ManagementLink and use it in jelly. The management page will be restricted to `ADMINISTER`.
...
|selenium-plugin
|No change
|getRequiredPermission() is overrided with a custom permission in the ManagementLink
|===
...
* link:https://github.com/jenkinsci/jenkins/pull/4483[PR with proposed changes and tests]
