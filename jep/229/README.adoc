...
and a link:https://github.com/jenkinsci/log-cli-plugin/blob/b1ee6375b940508709ff64cd8e154ad99eb270ce/.github/workflows/cd.yaml[GitHub Action workflow]
running link:https://github.com/jglick/jenkins-maven-cd-action/blob/3f1f2fa2739ce0747de9a2545aee5d54917a7abb/run.sh#L4[`mvn deploy`]
with a link:https://github.com/jglick/jenkins-maven-cd-action/blob/3f1f2fa2739ce0747de9a2545aee5d54917a7abb/settings.xml[bot token for Artifactory].
...
link:https://github.com/jglick/verify-ci-status-action/blob/7ed33273673b4d6d9d8b0533166dfda072b96807/run.sh[verifies that the Jenkins build passed]
whether the deployment was triggered
link:https://github.com/jenkinsci/log-cli-plugin/blob/b1ee6375b940508709ff64cd8e154ad99eb270ce/.github/workflows/cd.yaml#L2-L4[automatically or manually].
...
The Jenkins project’s own link:https://github.com/jenkinsci/core-changelog-generator[core-changelog-generator] could serve as an alternative,
though this is targeted at Jenkins core rather than components/plugins,
and to minimize disruption to repository maintainers it would be preferable to retain the exact behavior of Release Drafter.
...
The link:https://github.com/jglick/verify-ci-status-action[verify-ci-status] Action and
the link:https://github.com/jglick/jenkins-maven-cd-action[jenkins-maven-cd] Action
implement most of the logic of the CD process.
(These should be moved to the `jenkins-infra` organization when approved.)
