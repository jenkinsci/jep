...
and a link:https://github.com/jenkinsci/log-cli-plugin/blob/d041abc4ff822401430b3bc00bf5da31f1d6e8d9/.github/workflows/release.yaml#L37-L49[GitHub Action definition]
running link:https://github.com/jenkinsci/log-cli-plugin/blob/d041abc4ff822401430b3bc00bf5da31f1d6e8d9/.github/workflows/release.sh#L5[`mvn deploy`]
with a link:https://github.com/jenkinsci/log-cli-plugin/blob/d041abc4ff822401430b3bc00bf5da31f1d6e8d9/.github/workflows/settings.xml[bot token for Artifactory].
...
link:https://github.com/jenkinsci/log-cli-plugin/blob/d041abc4ff822401430b3bc00bf5da31f1d6e8d9/.github/workflows/release.sh#L6-L10[creates a (lightweight) Git tag and publishes the release].

In the future there may also be a new Action or other tooling wrapping most of `release.yaml`, `release.sh`, and `settings.xml` to avoid duplicating shell scripts across repositories.
...
and creates an Artifactory bot account with permissions to only the listed paths
(via a synthetic Artifactory group),
then generates an access token for that account
and saves it to the GitHub secrets `ARTIFACTORY_USERNAME` and `ARTIFACTORY_TOKEN` in the corresponding repository.
