...
* *Source control driven*:
    Depoloyments are triggered by changes to the source repository and are otherwise no-touch for users.
* *Assumes usage of current CI tools*:
    For simplicity and stablity this system assumes plugin already uses `buildPlugin` and `incrementals`.
...
====
*(bitwiseman)* 
There's a long thread here: https://github.com/jenkinsci/jep/pull/244#discussion_r293534268 that needs to be integrated into this JEP.
It seems to point to significantly simpler potential design.  
====
