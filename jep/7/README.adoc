...
The plugins will be also marked for adoption so that users can step up and take over maintenance without a 2 weeks waiting delay.
...
* Announcement blogpost on jenkins.io with the user-focused overview of the change and the delcivery plan
...
* If there are any unbpublished security issues related to the depublished plugins,
...
A pull request is submitted to the Jenkins, with a cleanup of the code and functionality slated for deletion.
...
there are contributors who take ownership of a plugin and migrate it to the new runtime.
