= JEP-0000: Publishing incremental commits as Maven releases
:toc: preamble
:toclevels: 3
ifdef::env-github[]
:tip-caption: :bulb:
...
[quote]
____
Plugin and core tooling should use a consistent format for defining incremental built versions,
such as: `<artifactId>-<major>.<minor>.<patch>-<sha1>.jar`.
The specific format is not required for this document.
____

This JEP proposes to specify that format, and the accompanying tooling.
...
There is some risk would a developer would blindly run `versions:display-dependency-updates`
