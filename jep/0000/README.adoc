...
One mitigation is to slow down the frequency at which the update center is automatically checked, currently one day.
...
which introduces some issues of token expiry and possible theft by repository owners,
...
[source,bash]
----
repo=$(curl -s -H "Authorization: Bearer $TOKEN" https://api.github.com/installation/repositories | jq -r '.repositories[0].full_name')
----
...
curl -H "Authorization: Bearer $TOKEN" -d '{"ref":"refs/tags/'$tag'","sha":"'$sha'"}' https://api.github.com/repos/$repo/git/refs
curl -H "Authorization: Bearer $TOKEN" -X DELETE https://api.github.com/repos/$repo/git/refs/tags/$tag
----
