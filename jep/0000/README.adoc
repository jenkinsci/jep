...
It is not actually used by any part of Jenkins Core itself.
It is also inherently risky from a security perspective:

* XML is overcomplicated and the XML parsing might be subject to XXE, maybe allowing clever attacks to do things like load `../../../credentials.xml`.
* Even if you make sure it is being fed a plain old self-contained XML document, you are still taking user-controlled strings and using them to look up and invoke Java methods, which is subject to abuse in numerous ways.
...
Instead, we should remove the Digester dependency from Jenkins Core and the plugins that actually use Digester should handle these security concerns themselves.
