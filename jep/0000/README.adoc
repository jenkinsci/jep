...
Some of these fragment views can leak information when dispatched directly, though many fragments will err early on due to referencing uninitialized variables.
...
This defined a specific servlet request attribute that would be used to storing the current request's dispatch filter status along with an API for manipulating that attribute.
